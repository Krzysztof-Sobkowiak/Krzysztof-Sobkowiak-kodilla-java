...
class BoardTestSuite {
    ...
    @DisplayName("Calculating average the number of days from the task request to the current day")
    @Test
    void testAddTaskListAverageWorkingOnTask(){
        ...
        List<Long> numberOfDaysFromTheTaskRequestToCurrentDay = project.getTaskLists().stream()
                .filter(ListInProgress::contains)
                .flatMap(taskList -> taskList.getTasks().stream())
                .map(task -> task.getCreated().until(LocalDate.now(), DAYS))
                .collect(toList());

        double average = LongStream.range(0, numberOfDaysFromTheTaskRequestToCurrentDay.size())
                .map(n -> numberOfDaysFromTheTaskRequestToCurrentDay.get((int) n))
                .average().getAsDouble();
